  ...
  useEffect(() => {
    ...
    const initializeExtractor = async () => {
      ...
      try {
        ...
        // Use the proxy endpoint for external PDFs
  ...
  const extractText = async (instance: any) => {
    ...
    try {
      ...
      for (let batch = 0; batch < batches; batch++) {
        ...
        for (let pageNum = startPage; pageNum <= endPage; pageNum++) {
          try {
            ...
            console.log(`[PDF Extractor] Extracted text from page ${pageNum}: ${text}`);
      ...
      try {
        ...
        console.log(`[PDF Extractor] Extracted text sample:`, Object.keys(extractedText).length, 'characters');
        
        const apiUrl = `/files/${fileId}/save-text`;
        ...
        console.log('[PDF Extractor] Text saved successfully:', response);
        extractionStatusRef.current = 'completed';
